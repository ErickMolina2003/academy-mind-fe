<template>
  <v-main style="padding-left: 0%" class="mx-3">
    <v-card>
      ...
      >
        <span
          >Carga del {{ currentPeriod.numberPeriod }} Período Académico
          {{ currentPeriod.year }}</span
        >
      </v-card-title>
      ...
      >
        ...
        <tbody>
          <tr v-for="item in paginatedCargaPeriodo" :key="item">
            ...
            <td class="text-left pa-0 px-2">
              {{ item.idTeacher.employeeNumber }}
            </td>
            <td class="text-left pa-0 px-2">
              {{ item.idTeacher.user.firstName }}
              {{ item.idTeacher.user.firstLastName }}
            </td>
            ...
            <td class="text-center pa-0 px-2">
...
async function getAcademicCharge() {
  const response = await sectionService.getAcademicCharge(
        ...
        table: {
          ...
          body: [
            [
              ...
              {
                text: "Cant. de estudiantes matriculados",
                style: "tableHeader",
              },
            ...
            ...academicCharge.value.map((item) => [
              ...
              {
                text: `${item.idTeacher.user.firstName} ${item.idTeacher.user.secondName} ${item.idTeacher.user.firstLastName} ${item.idTeacher.user.secondLastName}`,
                style: "tableContent",
              },
