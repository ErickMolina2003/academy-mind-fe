    ...
    <table>
      ...
      <tbody>
        <tr v-for="teacher in displayedTeachers" :key="teacher.employeeNumber">
          <td>{{ teacher.teacher.employeeNumber }}</td>
          <td>
            {{
              `${teacher.teacher.user.firstName} ${teacher.teacher.user.secondName} ${teacher.teacher.user.firstLastName} ${teacher.teacher.user.secondLastName}`
            }}
          </td>
          <td>{{ teacher.teacher.institutionalEmail }}</td>
          <td>
            <v-btn
              color="primary"
              @click="showClassDetails(teacher.teacher)"
              class="mr-2"
              >Reiniciar Clave</v-btn
            >
          </td>
        </tr>
      </tbody>
    ...
    <v-dialog v-model="showModal" persistent max-width="600">
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-container>
            ...
            <v-row>
              <v-col cols="12">
                <p class="mb-2">
                  <strong>Nombre:</strong> {{ selectedTeacher.user.firstName }}
                  {{ selectedTeacher.user.secondName }}
                  {{ selectedTeacher.user.firstLastName }}
                  {{ selectedTeacher.user.secondLastName }}
                </p>
                <p class="mb-2">
                  <strong>Correo Institucional:</strong>
                  {{ selectedTeacher.institutionalEmail }}
                </p>
                <p class="mb-2">
                  <strong>Número de empleado:</strong>
                  {{ selectedTeacher.employeeNumber }}
                </p>
                <p class="mb-2">
                  <strong>DNI:</strong> {{ selectedTeacher.user.dni }}
                </p>
                <p class="mb-2">
                  <strong>Email Personal:</strong> {{ selectedTeacher.email }}
                </p>
              </v-col>
            </v-row>
          </v-container>
        ...
        <v-card-actions class="fixed-footer">
          <v-spacer></v-spacer>
          <v-btn color="blue darken-4" text @click="showModal = false"
            >Cerrar</v-btn
          >
          <v-btn
            color="blue darken-4"
            @click="resetPassword(selectedTeacher.user.dni)"
            >Reiniciar Clave</v-btn
          >
        </v-card-actions>
      </v-card>
...
const career = ref(store.user.teacher.teachingCareer[0].centerCareer.career.id);
const center = ref(
  store.user.teacher.teachingCareer[0].centerCareer.regionalCenter.id
);
const userEmployeeNumber = ref(store.user.teacher.employeeNumber);
...
const getTeachers = async () => {
  ...
  teachers.value = originalTeachers.value.filter(
    (teacher) => teacher.teacher.employeeNumber !== userEmployeeNumber.value
  );
