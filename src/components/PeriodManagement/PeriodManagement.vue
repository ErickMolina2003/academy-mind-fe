    ...
    <v-dialog v-model="registrationModalOpen" max-width="450px">
      <v-card class="pa-5">
          ...
          <v-row class="justify-center">
            <v-col cols="12">
              <v-text-field
                ...
                label="Fecha de inicio de matricula"
                type="date"
                :rules="[required, validateRegistration]"
              ></v-text-field>
            </v-col>
          </v-row>
        </v-form>
        <v-card-actions class="fixed-footer">
          ...
          >
          <v-btn
            @click="confirmRegistrationDate"
            :disabled="!isFormRegistrationValid"
            >Aceptar</v-btn
          >
        </v-card-actions>
      </v-card>
...
async function getCancelationsDates() {
  ...
  if (response.periods[0]?.exceptionalCancelationStarts) {
    cancelStartDate.value = new Date(
      response.periods[0].exceptionalCancelationStarts
    )
      .toISOString()
      .split("T")[0];
    cancelEndDate.value = new Date(
