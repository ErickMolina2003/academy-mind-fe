<template>
    <div>
        ...
        <v-table class="mx-6 mt-4" fixed-header>
            <thead>
                <tr>
                    ...
                    <th v-if="periodToModify.idStatePeriod?.name !== 'Finalizado'" class="text-left pa-0">
                        Acciones
                    </th>
                    <th v-if="periodToModify.idStatePeriod?.name === 'Matricula'" class="text-left">
                        Fechas de Matricula
                    </th>
            ...
            <tbody>
                <tr v-for="period in periods" class="text-left">
                    ...
                    <td v-if="periodToModify.idStatePeriod?.name !== 'Finalizado'">
                        <v-row v-if="(period.id === periodToModify.id &&
                    ...
                    <td v-if="periodToModify.idStatePeriod?.name === 'Matricula' ||
                            periodToModify.idStatePeriod?.name === 'En curso'
                            ">
                        <v-btn v-if="period.idStatePeriod.name === 'Matricula'" @click="dialogDates = true">Mostrar Fechas
                            de Matrícula</v-btn>
                        <v-btn v-if="period.idStatePeriod.name === 'En curso' &&
                            !cancelStartDate &&
                            !cancelEndDate
                            " @click="cancelDatesModalOpen = true">Establecer Fechas</v-btn>

                        <div v-if="period.idStatePeriod.name === 'En curso' &&
                            cancelStartDate &&
                            cancelEndDate
                            ">
            ...
            <v-card class="pa-3">
                ...
                <v-form ref="form" v-model="isFormValid">
                    <v-row>
                        ...
                        <v-col cols="6">
                            <v-text-field v-model="cancelEndDate" :disabled="!cancelStartDate"
                                label="Fecha final de cancelaciones" type="date"
                                :rules="[required, validateEndDate]"></v-text-field>
                        </v-col>
                    </v-row>
                </v-form>
                <v-card-actions class="fixed-footer">
                    <v-btn class="me-3" @click="
        ...
        <!-- Modal para establecer fechas de inicio de matricula -->
        <v-dialog v-model="registrationModalOpen" max-width="450px">
            <v-card class="pa-5">
                <v-card-title>Establecer Fecha de Inicio de Matricula</v-card-title>

                <v-form ref="form" v-model="isFormRegistrationValid">
                    <v-row class="justify-center">
                        <v-col cols="12">
                            <v-text-field v-model="registrationDate" label="Fecha de inicio de matricula" type="date"
                                :rules="[required, validateRegistration]"></v-text-field>
                        </v-col>
                    </v-row>
                </v-form>
                <v-card-actions class="fixed-footer">
                    ...
                    registrationDate = null">Cancelar</v-btn>
                    <v-btn @click="confirmRegistrationDate" :disabled="!isFormRegistrationValid">Aceptar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
const isFormRegistrationValid = ref(false);
...
async function confirmAndChange() {
    if (chosenState.value === 'Matricula') {
